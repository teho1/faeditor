...
3. Tab **1. Studio Sets**: open a User/Preset slot (loads it into Temporary). Optional **Scan User Names**.
4. Tab **2. Change Tone**: pick a part → **Change** → double-click an instrument (writes live to the FA).
5. **Push Temp** if you want a full Temporary rewrite. To keep a **User** slot permanently, use **Write** on the FA itself (SysEx only edits Temporary).
